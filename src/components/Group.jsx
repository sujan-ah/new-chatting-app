import React from "react";

const Group = () => {
  return (
    <div className="mt-14 rounded-2xl p-10 h-[347px] overflow-y-scroll shadow-md">
      <h1 className="font-nunito font-bold text-lg">Groups Request</h1>
      <div className="flex justify-between mt-4 border-b pb-2.5 items-center">
        <div>
          <img src="images/groupimg.png" className="w-16 h-16 rounded-[50%]" />
        </div>
        <div>
          <h1 className="font-nunito font-bold text-base">Friends Reunion</h1>
          <p className="font-nunito font-semibold text-sm opacity-60">
            Hi Guys, Wassup!
          </p>
        </div>
        <div>
          <p className="bg-primary text-white font-nunito font-bold text-lg rounded p-1	">
            Accept
          </p>
        </div>
      </div>

      <div className="flex justify-between mt-4 border-b pb-2.5 items-center">
        <div>
          <img src="images/groupimg.png" className="w-16 h-16 rounded-[50%]" />
        </div>
        <div>
          <h1 className="font-nunito font-bold text-base">Friends Reunion</h1>
          <p className="font-nunito font-semibold text-sm opacity-60">
            Hi Guys, Wassup!
          </p>
        </div>
        <div>
          <p className="bg-primary text-white font-nunito font-bold text-lg rounded p-1	">
            Accept
          </p>
        </div>
      </div>
      <div className="flex justify-between mt-4 border-b pb-2.5 items-center">
        <div>
          <img src="images/groupimg.png" className="w-16 h-16 rounded-[50%]" />
        </div>
        <div>
          <h1 className="font-nunito font-bold text-base">Friends Reunion</h1>
          <p className="font-nunito font-semibold text-sm opacity-60">
            Hi Guys, Wassup!
          </p>
        </div>
        <div>
          <p className="bg-primary text-white font-nunito font-bold text-lg rounded p-1	">
            Accept
          </p>
        </div>
      </div>
      <div className="flex justify-between mt-4 border-b pb-2.5 items-center">
        <div>
          <img src="images/groupimg.png" className="w-16 h-16 rounded-[50%]" />
        </div>
        <div>
          <h1 className="font-nunito font-bold text-base">Friends Reunion</h1>
          <p className="font-nunito font-semibold text-sm opacity-60">
            Hi Guys, Wassup!
          </p>
        </div>
        <div>
          <p className="bg-primary text-white font-nunito font-bold text-lg rounded p-1	">
            Accept
          </p>
        </div>
      </div>
      <div className="flex justify-between mt-4 border-b pb-2.5 items-center">
        <div>
          <img src="images/groupimg.png" className="w-16 h-16 rounded-[50%]" />
        </div>
        <div>
          <h1 className="font-nunito font-bold text-base">Friends Reunion</h1>
          <p className="font-nunito font-semibold text-sm opacity-60">
            Hi Guys, Wassup!
          </p>
        </div>
        <div>
          <p className="bg-primary text-white font-nunito font-bold text-lg rounded p-1	">
            Accept
          </p>
        </div>
      </div>
      <div className="flex justify-between mt-4 border-b pb-2.5 items-center">
        <div>
          <img src="images/groupimg.png" className="w-16 h-16 rounded-[50%]" />
        </div>
        <div>
          <h1 className="font-nunito font-bold text-base">Friends Reunion</h1>
          <p className="font-nunito font-semibold text-sm opacity-60">
            Hi Guys, Wassup!
          </p>
        </div>
        <div>
          <p className="bg-primary text-white font-nunito font-bold text-lg rounded p-1	">
            Accept
          </p>
        </div>
      </div>
      <div className="flex justify-between mt-4 border-b pb-2.5 items-center">
        <div>
          <img src="images/groupimg.png" className="w-16 h-16 rounded-[50%]" />
        </div>
        <div>
          <h1 className="font-nunito font-bold text-base">Friends Reunion</h1>
          <p className="font-nunito font-semibold text-sm opacity-60">
            Hi Guys, Wassup!
          </p>
        </div>
        <div>
          <p className="bg-primary text-white font-nunito font-bold text-lg rounded p-1	">
            Accept
          </p>
        </div>
      </div>
      <div className="flex justify-between mt-4 border-b pb-2.5 items-center">
        <div>
          <img src="images/groupimg.png" className="w-16 h-16 rounded-[50%]" />
        </div>
        <div>
          <h1 className="font-nunito font-bold text-base">Friends Reunion</h1>
          <p className="font-nunito font-semibold text-sm opacity-60">
            Hi Guys, Wassup!
          </p>
        </div>
        <div>
          <p className="bg-primary text-white font-nunito font-bold text-lg rounded p-1	">
            Accept
          </p>
        </div>
      </div>
      <div className="flex justify-between mt-4 border-b pb-2.5 items-center last:border-0">
        <div>
          <img src="images/groupimg.png" className="w-16 h-16 rounded-[50%]" />
        </div>
        <div>
          <h1 className="font-nunito font-bold text-base">Friends Reunion</h1>
          <p className="font-nunito font-semibold text-sm opacity-60">
            Hi Guys, Wassup!
          </p>
        </div>
        <div>
          <p className="bg-primary text-white font-nunito font-bold text-lg rounded p-1	">
            Accept
          </p>
        </div>
      </div>
    </div>
  );
};

export default Group;
